...
void main(void)
{
    ...
    float s = length(screenSunPos - positionScreen);

    float lightStrength = sunStrength * exp(-sunDecay * s);
    vec4 lightColor = vec4(lightStrength, lightStrength, lightStrength, 1);
